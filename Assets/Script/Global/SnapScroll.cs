...
public class SnapScroll : UIBehaviour, IEndDragHandler, IBeginDragHandler
{
    ...
    public ScrollRectEx scrollRect; // the scroll rect to scroll
    ...
    {
        StopCoroutine(SnapRect(direction == SnapDirection.Horizontal ? scrollRect.horizontalNormalizedPosition : scrollRect.verticalNormalizedPosition, 0)); // if we are snapping, stop for the next input
    }

    public void OnEndDrag(PointerEventData eventData)
    {
        StartCoroutine(SnapRect(direction == SnapDirection.Horizontal ? scrollRect.horizontalNormalizedPosition : scrollRect.verticalNormalizedPosition, 0)); // simply start our coroutine ( better than using update )
    }

    public void NextSnap(int index)
    {
        ...
        StartCoroutine(SnapRect(nextStartNormal,index));
    }

    public int GetCurrentitemCount()
    {
        float delta = 1f / (float)(itemCount - 1);
        int count = direction == SnapDirection.Horizontal ? Mathf.RoundToInt(scrollRect.horizontalNormalizedPosition / delta) : Mathf.RoundToInt(scrollRect.verticalNormalizedPosition / delta); 
        if (count <= 0)
            count = 0;
    ...
    public void PreviousSnap(int index)
    {
        //index++;
        float nextStartNormal = direction == SnapDirection.Horizontal ? scrollRect.horizontalNormalizedPosition : scrollRect.verticalNormalizedPosition;
        nextStartNormal -= (1f / (float)(itemCount - 1)) / 1.5f;
        //nextStartNormal -= index;
        StartCoroutine(SnapRect(nextStartNormal,index));
    }

    private IEnumerator SnapRect(float startNormal,int index)
    {
        if (scrollRect == null)
        ...
        if(index > 0)
        {
            ...
            float nextStartNormal = direction == SnapDirection.Horizontal ? scrollRect.horizontalNormalizedPosition : scrollRect.verticalNormalizedPosition;
            nextStartNormal -= (1f / (float)(itemCount - 1)) / 1.5f;
            StartCoroutine(SnapRect(nextStartNormal, index - 1));
        }
