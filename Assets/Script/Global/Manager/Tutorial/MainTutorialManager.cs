...
public class MainTutorialManager : MonoBehaviour
{
        ...
        if (!haveToWait && !waitForClick)
        {
            ...
            switch (index)
            {
                ...
                case 14://클릭해데스
                    ...
                    waitForClick = true;
                    StartCoroutine(DelayWhithActive(0.2f, 0, -64, 200, 150));
                    StartCoroutine(DelayWithClickEvent());
                    break;
                case 15:
                    SetTargetSelector(0, -219, 240, 60);
                    break;
                case 16:
                    SetTargetSelector(0, -35, 240, 300);
                    break;
            }
            talkText.Setup(talkManager.TalkData[1100 + index].GetTalk());
            index++;
